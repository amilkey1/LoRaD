#' This function standardizes the estimation sample
#'
...
#' @return A new data frame consisting of standardized estimation sample with log kernel in last column
#' @export 
#'
standardize_estimation_sample <- function(standardinfo, y) {
 ...
 inversesqrts_matrix <- matrix(unlist(invsqrts), nrow =p, ncol = p)
  ...
  mean_vec <- matrix(unlist(col_means), nrow=p, ncol=1)
