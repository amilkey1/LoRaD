...
In order to use the LoRaD method, we need to export the sampled parameter values along with the unnormalized log posterior to a text file.The `permuted=TRUE` setting causes samples from all 3 chains to be merged.
...
lorad_estimate(paramsH0, colspecH0, 0.5, "random", 0.1)
```
...
lorad_estimate(paramsH1, colspecH1, 0.5, "random", 0.1)
```
